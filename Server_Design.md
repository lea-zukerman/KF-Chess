...
| **הבחירה** | **NATS core** | **NATS JetStream** |
| חלופות שנשקלו | Redis Pub/Sub | Kafka, RabbitMQ |

### 3.4.1 מדוע NATS לשני התפקידים

**מערכת אחת, שני מצבי עבודה.** \u200FNATS core הוא pub/sub טהור בזיכרון;
JetStream היא שכבת עמידות **מעל אותו שרת** — אותו חיבור, אותה ספריית לקוח,
אותו קונטיינר. במקום שתי תשתיות נפרדות לתחזק, מפעילים NATS עם JetStream
ומקבלים את שני הערוצים.

| | NATS + JetStream | Kafka |
|---|---|---|
| התקנה | בינארי יחיד, קונטיינר אחד | brokers + תיאום, כבד יותר |
| latency | מיקרו־שניות | מילישניות |
| שימור ארוך טווח | לא ייעודו | חזק — יומן שמור לאורך זמן |
| התאמה לנו | ✅ | מעבר לצורך |

**מדוע לא Kafka:** הוא מצוין ליומן אירועים ארוך טווח שנקרא שוב ושוב על ידי
צרכנים רבים בקצב שונה — למשל צנרת אנליטיקה. אצלנו התור נועד למשימה אחת
צרה: להעביר `game_over` לכתיבה, ולשכוח. JetStream מספיק, ועולה הרבה פחות
בתפעול. אם בהמשך תיבנה צנרת אנליטיקה על היסטוריית הצעדים (סעיף 4.4),
**שם** Kafka יהיה הבחירה הנכונה.

**מדוע לא Redis Pub/Sub לאפיק האירועים:** אפשרי, ו‑Redis כבר קיים במערכת —
אבל הוא היה כובל את התקשורת בין השירותים לאותו מופע שמחזיק את הסטייט
הזמני, ומערבב שני תפקידים שנרצה להרחיב בנפרד.
...
**תקשורת** — **NATS** (core ל‑pub/sub, \u200FJetStream לתור העמיד; סעיף 3.4.1)
מופיע כאן לשלמות התמונה אבל **אינו בסיס נתונים**: הוא מעביר הודעות ולא
משמש כמקור אמת. \u200FJetStream אמנם שומר הודעות לדיסק, אך ככלי מעבר עד לאישור
העיבוד — לא כמקום שממנו קוראים דירוגים.
...
#### 5.2.1 מי מבטל המתנה שלא נענתה

הדרישה היא לוותר אחרי 60 שניות. היום זה `asyncio.wait_for` בתוך
`matchmaking.py`, עם `finally` שמסיר מהרשימה — נכון לחלוטין **כל עוד הכול
בתהליך אחד**.

**במבוזר זה נשבר:** אם הטיימר יושב בזיכרון של פוד Matchmaker והפוד מת
בשנייה ה‑30, בלוק ה‑`finally` לא ירוץ לעולם, והשחקן נשאר בתור ב‑Redis
לצמיתות. גרוע מכך — הוא עלול להשתדך מאוחר יותר בלי שיש מי שיודיע לו.
**הכלל: אין לשים deadline בזיכרון של תהליך שעלול למות.**

לכן האחריות מתפצלת לשתיים:

| | הבעלים | הנימוק |
|---|--------|--------|
| **להודיע לשחקן** | ה‑WS Gateway שמחזיק את החיבור | אם הוא מת, החיבור נופל ממילא והשחקן כבר לא ממתין — השאלה מתאיידת |
| **לנקות מ‑Redis** | **אף אחד** — הזמן | ראו למטה |

הניקוי הופך **לתכונה של הדאטה** ולא למשימה של תהליך: לצד ה‑sorted set
המדורג לפי ELO נשמר שני, מדורג לפי חותמת זמן. כל Matchmaker סורק מדי כמה
שניות את `ZRANGEBYSCORE waiting:time -inf (now-60)` ומסיר את מי שפג תוקפו
משתי הקבוצות.

הפעולה אידמפוטנטית — הסרת מי שכבר הוסר היא no-op — ולכן **אין צורך בבעלים,
בבחירת מנהיג או בתיאום**. כל פוד יכול לבצע אותה, ופוד שמת אינו מעכב דבר.

| התרחיש | מי מטפל |
|--------|---------|
| עברו 60 שניות והשחקן מחובר | ה‑Gateway מודיע; הסריקה מנקה |
| השחקן התנתק תוך כדי המתנה | ה‑Gateway מסיר מיד עם זיהוי הניתוק |
| **פוד ה‑Matchmaker מת** | **הסריקה מנקה — איש אינו נתקע** |

**מה שמשתנה בקוד:** `find_match` מתפצל. ההכנסה לתור והחיפוש נשארים
ב‑`Matchmaker` מול Redis; **ההמתנה** יוצאת מ‑`asyncio.wait_for` ועוברת
ל‑Gateway, שממתין להודעה על `match.assigned` או ל‑60 שניות — המוקדם מביניהם.
\u200F`_WaitingPlayer` ו‑`asyncio.Event` נעלמים.
...
| `kungfu_chess/bus/event_bus.py` | NATS core | אותו pattern, מימוש רשתי |
...
**מי מזהה את הפער — השרת, ברוב המקרים.** בתוך חיבור TCP חי הודעה אינה
נעלמת בשקט: או שהיא מגיעה, או שהחיבור נשבר. לכן אם לקוח פספס עדכון בזמן
שהחיבור היה פתוח, זה משום ש**השרת בחר לא לשלוח** — וזה מידע שיש לו:

| מה קרה | איך השרת יודע | מה הוא עושה |
|--------|----------------|--------------|
| התחברות או הצטרפות כצופה | פתיחת החיבור היא אירוע שלו | מצב מלא ביוזמתו (קיים היום ב‑`join`) |
| זרק עדכון בתור מלא (9.2) | הוא זה שזרק | מסמן "לא מסונכרן" ודוחף מצב מלא |
| הלקוח איטי, חוצץ השליחה מתמלא | `send`/`drain` מתארך | מפסיק דלתאות ושולח snapshot אחד |
| `send` נכשל | חריגה מפורשת | מסמן את החיבור לסנכרון |
| הלקוח עבר ל‑Gateway אחר | ל‑Gateway החדש אין היסטוריה | שולף snapshot מהשארד |

לכן המנגנון העיקרי הוא **דחיפה יזומה מהשרת**, לא בקשה מהלקוח.

**רשת הביטחון לשארית.** נשארים מקרים שהשרת עיוור אליהם — באג בלקוח,
middleware באמצע, ומעבר עתידי ל‑UDP שבו אובדן שקט הוא נורמלי. לכן:

1. **מספר סידורי לכל עדכון**, כך שניתן לזהות פער (41 ואז 43).
2. **דיווח `last_seen` מוצמד להודעות הלקוח** — הוא ממילא שולח צעד כל שתי
   שניות, ותוספת של שדה אחד עולה בתים בודדים. השרת משווה ומסיק לבד. עדיף
   על בקשת סנכרון מפורשת, שעולה סיבוב שלם.
3. **מצב מלא בכל התחברות** — כבר קיים ב‑`match.py` (`join` משדר מצב לפני
   הכניסה ללולאת ההודעות).

**מה זה לא מכסה.** כל האמור לעיל מטפל באובדן **בערוץ**, כאשר הלוח הסמכותי
עדיין קיים בשארד. אם **השארד עצמו נפל**, הלוח נעלם איתו (סעיף 9.1) ואין
ממי לבקש סנכרון — במקרה הזה המשחק מבוטל, לפי ההחלטה בסעיף 10.3.
...
| `nats` | אפיק האירועים ו‑JetStream לתור התוצאות (סעיף 3.4.1) |
| `gateway` | WS Gateway + לוגין; מנתב לשארד |
| `allocator` | בחירת שארד (round-robin או "הכי פחות עמוס") |
| `results-writer` | צורך `game_over` מ‑JetStream, כותב ELO |
| `shard-1`, `shard-2` | שני מופעים של שרת המשחק |

שמונה קונטיינרים. `nats` הוא קונטיינר יחיד שמשרת את שני הערוצים — זה בדיוק
היתרון התפעולי שבגללו נבחר בסעיף 3.4.1.

מה שזה מדגים: DB חיצוני משותף, סטייט משותף ב‑Redis, החלטת שיבוץ נפרדת
מהשידוך, ריצה של שני שארדים במקביל, ומסלול תוצאות אסינכרוני שאינו חוצה את
הנתיב החם. מה שזה **לא** מדגים ונשאר לתכנון בלבד: Agones, מולטי‑אזור,
ו‑autoscaling.
...
**שלב 4 — עמידות.** \u200FNATS JetStream לתוצאות, `Results Writer` נפרד עם idempotency
...
### 15.1 שתי בעיות נפרדות

הבלבול הנפוץ הוא להתייחס להצפנה כאל דבר אחד. אלה שני איומים שונים,
ודורשים מנגנונים שונים:

| האיום | המנגנון | המצב בקוד היום |
|-------|---------|-----------------|
| ה‑DB דולף — התוקף קורא סיסמאות מאוחסנות | **PBKDF2 עם salt**, 100,000 סבבים | ✅ קיים ב‑`db.py` |
| מאזין ברשת קורא סיסמה בתנועה | **TLS** | ❌ **חסר** — `LoginRequest` נשלח על `ws://` |

השורה השנייה היא הפער האמיתי: ה‑PBKDF2 מגן מצוין על מה שמאוחסן, ולא מגן
כלל על מה שעובר על החוט.
...
Let's Encrypt, עם חידוש אוטומטי) ולא קוד פייתון.

| שלב | מה נדרש |
|-----|---------|
| Docker Compose | אפשר לדלג — סביבה מקומית מבודדת |
| k3s | Ingress עם תעודה self-signed, לתרגול ההגדרה |
| פרודקשן | Ingress + cert-manager, תעודה אמיתית |

### 15.3 סשנים

הסיסמה נשלחת **פעם אחת בלבד**, בלוגין. משם ואילך מוחזק session token
ב‑Redis עם TTL, וההתחברות ב‑WebSocket מציגה טוקן ולא סיסמה (סעיף 5.1).
זה מצמצם את חלון החשיפה מכל הודעה להודעה אחת.

הטוקן עצמו הוא **אישור נושא** — מי שמחזיק בו הוא המשתמש. לכן הוא מחייב
TLS בדיוק כמו הסיסמה, ומחייב TTL קצר יחסית ואפשרות ביטול (מחיקת המפתח
מ‑Redis מנתקת סשן מיידית).

### 15.4 הצפנה בתוך הקלאסטר

**ההחלטה: לא בשלב זה.** רשת הקלאסטר מבודדת מהאינטרנט, ו‑Ingress הוא נקודת
הכניסה היחידה. אם תידרש הצפנה פנימית, הדרך הנכונה היא **service mesh**
(\u200FLinkerd) שמוסיף mTLS אוטומטי בין כל הפודים דרך sidecar — **בלי שינוי
קוד**. הוספה ידנית של TLS לכל חיבור אינה מומלצת.

### 15.5 מה שאינו מכוסה כאן

- **הגבלת קצב על לוגין.** אימות PBKDF2 עולה ~10ms CPU (סעיף 2.3), ולכן
  מתקפת ניחוש סיסמאות היא גם **מתקפת מניעת שירות זולה**: התוקף משלם
  בקשה, המערכת משלמת 10 מילישניות מעבד. נדרשת הגבלת קצב לפי IP ולפי
  חשבון ב‑API Gateway, לפני שהבקשה מגיעה ל‑Auth Service.
- **אימות הרשאות בתוך משחק** — קיים חלקית: `match.py` בודק תור ותפקיד
  (שחקן מול צופה), אך ההרשאה נשענת על כך שה‑Gateway קישר נכון בין חיבור
  למשתמש.
- ניהול סודות (סיסמאות DB, מפתחות) דרך Kubernetes Secrets, ולא במניפסט.
