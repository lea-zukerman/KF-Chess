...
|---|---|---|
| דוגמה | שיבוץ משחק, שינוי מצב | `game_over` → כתיבת ELO |
| הדרישה | latency מינימלי | **אסור לאבד הודעה** |
| אובדן הודעה | נסבל — יגיע עדכון הבא | בלתי קביל — דירוג נעלם |
...
באותו רגע — ההודעה פשוט נעלמת. לעדכון מצב זה בסדר, כי מיד יגיע הבא. לתוצאת
משחק ולדירוג זה אובדן דאטה. לכן תוצאות **חייבות** לעבור בתור עמיד, וזה גם
מה שמצריך את ה‑idempotency בסעיף 5.5.
...
### 4.3 כמה היסטוריה שומרים — החלטה מוצרית
